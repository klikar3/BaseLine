...
class ServerViewController extends \yii\web\Controller
{
    public function actionIndex($id)
    {
        ...
        $cmd = $connection
      	       ->createCommand("SELECT MAX([CaptureDate]) FROM ConfigData WITH (READPAST) WHERE [Server]=:srv and Name='affinity mask';");
   ...
   public function getWaitDataset($srvr,$dt)
    {
      ...
      try{  
        $dataset = (new \yii\db\Query())
        ->select('Waits.CaptureEnd, WaitTypes.WaitType, Waits.WaitCount, Waits.Percentage, SumWaitSecs')
 ...
 //                ['Setting', new JsExpression("function go() {window.location.assign(\"".Url::toRoute(['perf-counter-per-server/update','id' => $pcsid ])."\");}") ],
    ...
    public static function getBackground($stat = 'unknown') {
      ...
      switch ($stat) {
          ...
          case 'greenU':
              $bg = 'Gradient(lightgreen:black)'; break;
          case 'yellowU':
              $bg = 'Gradient(yellow:black)'; break;
          case 'redU':
              $bg = 'Gradient(orange:black)'; break;
          default:
